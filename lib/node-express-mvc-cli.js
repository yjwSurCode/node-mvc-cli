...
async function expressGenTs(destination, withAuth) {
    ...
    try {
        ...
        const result = await copyProjectFiles(destination, prjFolder);
 ...
 * @param message 
 */
async function fnLoadingByOra(fn, message) {
...
// const func = (destination, withAuth) => { return 1 };

// fnLoadingByOra(func, '123')
...
function copyProjectFiles(destination, prjFolder) {
    ...
    const dir = path.join(path.resolve(__dirname, '..'), prjFolder);
    console.log(source, 'source') // ! C:\material\NODE\node-express-cli\lib\project-files

    if (fs.existsSync(dir)) {
        console.error('Folder already exists!')
        spinner.stop('Folder already exists!!');
        return true;
    }
...
function getDepStrings(withAuth, tsRepo, redisRepo) {
    ...
    let devDependencies = 'nodemon sequelize mysql2 ' +
    ...
    //TODO 完善数据
    if (withAuth) {
        dependencies += 'http-status-codes ' + 'module-alias  express-async-errors jsonfile express-jwt ';
